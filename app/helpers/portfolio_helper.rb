module PortfolioHelper
  def render_block(block)
    render partial: "portfolios/shared/#{block.kind}_widget.html.erb",
           locals: { portfolio: block.portfolio }
  end
  ...
  def draw_tag(title, color: 'blue-grey')
    content_tag :div, class: 'col s12 m6 l6' do
      concat content_tag(
        :span,
        title,
        class: "label #{color} #{color}-darken-5 #{color}-text text-lighten-4"
      )
    end
  end
end
