module SearchPortfoliosService
  class << self
    ...
    def search_by_place
      ...
      @basic_search.where!('contacts.address_country = :place' \
                           ' OR contacts.address_city = :place',
                           place: @filters[:place])
    end
    ...
    def perform_featured_search
      @results[:featured] = @basic_search.where(featured: true)
                                         .order('RANDOM()').limit(2)
      @results[:general] = @basic_search.where(featured: false)
                                        .page(@filters[:page])
    end
  end
end
